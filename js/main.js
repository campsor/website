...
(function () {
  ...
  // Theme toggle: teal (default) <-> light, remembered per visitor
  const toggle = document.getElementById("themeToggle");
  if (toggle) toggle.addEventListener("click", () => {
    const next = document.documentElement.getAttribute("data-theme") === "tech-light" ? "tech" : "tech-light";
  ...
  if (gate) {
    const KEY = "cc-eligibility-v1";
    const accept = document.getElementById("gateAccept");
    const decline = document.getElementById("gateDecline");
    const panel = gate.querySelector(".gate__panel");

    accept.addEventListener("click", () => {
      try { localStorage.setItem(KEY, new Date().toISOString()); } catch (err) {}
      document.documentElement.setAttribute("data-gate", "ok");
      gate.remove();
    });

    decline.addEventListener("click", () => {
      panel.innerHTML = '<p class="eyebrow">Important notice</p>' +
        '<h2>Access restricted</h2>' +
        '<div class="gate__body gate__declined"><p>This website is available to professional, ' +
        'institutional and well-informed investors only. Thank you for your interest.</p></div>';
      panel.focus();
    });
    ...
    gate.addEventListener("keydown", (e) => {
      if (e.key !== "Tab") return;
      const f = [...gate.querySelectorAll("button, a[href]")].filter((el) => el.offsetParent !== null);
      if (!f.length) return;
  ...
  function renderTable(data) {
    ...
    data.years.forEach((y) => {
      ...
      let html = `<th scope="row">${y.year}</th>`;
      y.months.forEach((m) => { html += `<td>${fmt(m)}</td>`; });
      html += `<td class="perf-table__annual">${fmt(y.annual)}${y.ytd ? '<span class="ytd">YTD</span>' : ""}</td>`;
      tr.innerHTML = html;
      tbody.append(tr);
    });
  }
  ...
  // The rows are written into the HTML at build time (scripts/render_table.py), so they are
  // visible to crawlers and to browsers without JavaScript. Fall back to fetching the JSON only
  // if the table somehow arrived empty.
  if (table.tBodies[0].rows.length) {
    buildSeries();
    drawChart();
  } else {
    fetch("data/returns.json?v=" + Date.now(), { cache: "no-store" })
      .then((r) => { if (!r.ok) throw new Error(r.status); return r.json(); })
      .then((data) => { renderTable(data); buildSeries(); drawChart(); })
      .catch(() => {
        table.tBodies[0].innerHTML = '<tr><td colspan="14" style="text-align:left;color:var(--ink-3)">Track record temporarily unavailable.</td></tr>';
      });
  }
